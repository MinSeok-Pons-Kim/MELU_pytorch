...
def run(args, log_interval=100, verbose=True, save_path=None):
   ...
   # scheduler = torch.optim.lr_scheduler.StepLR(meta_optimiser, 5000, args.lr_meta_decay)
    ...
    for epoch in range(args.num_epoch):
        ...
        for step, batch in enumerate(dataloader_train):
            ...
            if step<1 or step%args.tasks_per_metaupdate!=0:
                continue
...
if __name__ == '__main__':
    args = parse_args()
    if not args.test:
        run(args, log_interval=100, verbose=True, save_path=None)
    else:
        utils.set_seed(args.seed)
        code_root = os.path.dirname(os.path.realpath(__file__))
